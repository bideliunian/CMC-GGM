...
working.path <- "~/work/CMC-GGM/Model_c"
save.path <- "~/work/CMC-GGM/Model_c/Results/p50"
...
S.linear = cov(data.linear)


#########################
## PART 3: Thresholding
###########################
est.prec.cmc.thres = vggm.thr(X=S.cmcopula, p = p, rho=10^-1, ridge = TRUE)$Theta0
est.prec.copula.thres = vggm.thr(X=S.copula, p = p, rho=10^-1, ridge = TRUE)$Theta0
est.prec.linear.thres = vggm.thr(X=S.linear, p = p, rho=10^-1, ridge = TRUE)$Theta0
...
roc.cmc.bglasso <- foreach(l = 1:L, .combine="cbind", .packages=c("matrixcalc","igraph","BB")) %dopar% {
  G.bglasso <- bglasso(S = S.cmcopula+10^-1*rho.cmc*diag(M*p), lambda = lambda.cmc[l], n = n, p = p,
...
roc.copula.bglasso <- foreach(l = 1:L, .combine="cbind", .packages="matrixcalc") %dopar% {
  G.bglasso <- bglasso(S = S.copula+10^-1*rho.copula*diag(M*p), lambda = lambda.copula[l], n = n, p = p,
                       tol.Theta=tol.Theta, tol.beta=tol.beta,
                       tol.beta_k=tol.beta_k, maxit.Theta=maxit.Theta, maxit.beta=maxit.beta,
                       maxit.beta_k=maxit.beta_k, Theta=NULL, Theta.inv= NULL, method=method)$alpha
  TFPR.l <- roc(est = G.bglasso, pop = G, thres = 0)
...
roc.linear.bglasso <- foreach(l = 1:L, .combine="cbind", .packages="matrixcalc") %dopar% {
  G.bglasso <- bglasso(S = S.linear+10^-1*rho.linear*diag(M*p), lambda = lambda.linear[l], n = n, p = p,
                       tol.Theta=tol.Theta, tol.beta=tol.beta,
                       tol.beta_k=tol.beta_k, maxit.Theta=maxit.Theta, maxit.beta=maxit.beta,
                       maxit.beta_k=maxit.beta_k, Theta=NULL, Theta.inv= NULL, method=method)$alpha
  TFPR.l <- roc(est = G.bglasso, pop = G, thres = 0)
...
save(df, file=paste(save.path,"/ROC.c.050.gauss.seed",rand.seed,".Rdata",sep=""))
...
save(df, file=paste(save.path,"/ROC.c.050.copula.seed",rand.seed,".Rdata",sep=""))
...
lambda.cmc = exp(seq(log(lambda.cmc.max), log(10^-2*lambda.cmc.max), length = L))
lambda.copula = exp(seq(log(lambda.copula.max), log(10^-2*lambda.copula.max), length = L))
lambda.linear = exp(seq(log(lambda.linear.max), log(10^-2*lambda.linear.max), length = L))
...
## bglasso with cmcopula transformation
roc.cmc.bglasso <- foreach(l = 1:L, .combine="cbind", .packages=c("matrixcalc","igraph","BB")) %dopar% {
  G.bglasso <- bglasso(S = S.cmcopula+10^-2*rho.cmc*diag(M*p), lambda = lambda.cmc[l], n = n, p = p,
                       tol.Theta=tol.Theta, tol.beta=tol.beta,
                       tol.beta_k=tol.beta_k, maxit.Theta=maxit.Theta, maxit.beta=maxit.beta,
                       maxit.beta_k=maxit.beta_k, Theta=NULL, Theta.inv= NULL, method=method)$alpha
  TFPR.l <- roc(est = G.bglasso, pop = G, thres = 0)
...
roc.copula.bglasso <- foreach(l = 1:L, .combine="cbind", .packages="matrixcalc") %dopar% {
  G.bglasso <- bglasso(S = S.copula+10^-2*rho.copula*diag(M*p), lambda = lambda.copula[l], n = n, p = p,
                       tol.Theta=tol.Theta, tol.beta=tol.beta,
                       tol.beta_k=tol.beta_k, maxit.Theta=maxit.Theta, maxit.beta=maxit.beta,
                       maxit.beta_k=maxit.beta_k, Theta=NULL, Theta.inv= NULL, method=method)$alpha
  TFPR.l <- roc(est = G.bglasso, pop = G, thres = 0)
...
roc.linear.bglasso <- foreach(l = 1:L, .combine="cbind", .packages="matrixcalc") %dopar% {
  G.bglasso <- bglasso(S = S.linear+10^-2*rho.linear*diag(M*p), lambda = lambda.linear[l], n = n, p = p,
                       tol.Theta=tol.Theta, tol.beta=tol.beta,
                       tol.beta_k=tol.beta_k, maxit.Theta=maxit.Theta, maxit.beta=maxit.beta,
                       maxit.beta_k=maxit.beta_k, Theta=NULL, Theta.inv= NULL, method=method)$alpha
  TFPR.l <- roc(est = G.bglasso, pop = G, thres = 0)
...
save(df, file=paste(save.path,"/ROC.c.050.cmcopula.seed",rand.seed,".Rdata",sep=""))
