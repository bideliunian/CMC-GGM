...
save.path <- "~/work/CMC-GGM/Model_b/Results/p50"
...
roc.cmc.bglasso <- foreach(l = 1:L, .combine="cbind", .packages=c("matrixcalc","igraph","BB")) %dopar% {
  G.bglasso <- bglasso(S = S.cmcopula+10^-3*rho.cmc*diag(M*p), lambda = lambda.cmc[l], n = n, p = p,
...
roc.copula.bglasso <- foreach(l = 1:L, .combine="cbind", .packages="matrixcalc") %dopar% {
  G.bglasso <- bglasso(S = S.copula+10^-3*rho.copula*diag(M*p), lambda = lambda.copula[l], n = n, p = p,
                    tol.Theta=tol.Theta, tol.beta=tol.beta,
                    tol.beta_k=tol.beta_k, maxit.Theta=maxit.Theta, maxit.beta=maxit.beta,
                    maxit.beta_k=maxit.beta_k, Theta=NULL, Theta.inv= NULL, method=method)$alpha
  TFPR.l <- roc(est = G.bglasso, pop = G, thres = 0)
...
roc.linear.bglasso <- foreach(l = 1:L, .combine="cbind", .packages="matrixcalc") %dopar% {
  G.bglasso <- bglasso(S = S.linear+10^-3*rho.linear*diag(M*p), lambda = lambda.linear[l], n = n, p = p,
                    tol.Theta=tol.Theta, tol.beta=tol.beta,
                    tol.beta_k=tol.beta_k, maxit.Theta=maxit.Theta, maxit.beta=maxit.beta,
                    maxit.beta_k=maxit.beta_k, Theta=NULL, Theta.inv= NULL, method=method)$alpha
  TFPR.l <- roc(est = G.bglasso, pop = G, thres = 0)
...
save(df, file=paste(save.path,"/ROC.b.050.gauss.seed",rand.seed,".Rdata",sep=""))
...
save(df, file=paste(save.path,"/ROC.b.050.copula.seed",rand.seed,".Rdata",sep=""))
...
save(df, file=paste(save.path,"/ROC.b.050.cmcopula.seed",rand.seed,".Rdata",sep=""))
