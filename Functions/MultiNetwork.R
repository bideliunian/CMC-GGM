...
#' @import stabs
#' @export
multivarNetwork <- function(X, select=c("none", "1se", "min" ,"stabsel"), lambda.max=NULL,
  ...
  if(is.null(lambda.max)){
    lambda.max <- max(sapply(training_sets, function(data) {
      return(max(sqrt(rowsum(as.numeric(crossprod(data$X, data$Y)^2), group))/n))
    }))
  }
  ...
  networks.or <- apply(coefficients, 2, function(beta) {
    net <- matrix(beta != 0,p,p)
    net.or <- pmax(net,t(net))
    net.or
  })

  networks.and <- apply(coefficients, 2, function(beta) {
    net <- matrix(beta != 0,p,p)
    net.and <- pmin(net,t(net))
    net.and
  })
  
  return(list(coefficients = coefficients, networks.or = networks.or, 
              networks.and = networks.and, penalties = lambda))
}
