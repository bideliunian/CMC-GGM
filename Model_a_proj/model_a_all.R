...
######################
U.df = read.csv(file=paste(read.path,"/proj_subspace_model1_exp_d03.csv",sep=""), header = FALSE)
U.list = list()
for (i in 1:p) {
...
lambda.pcmc = exp(seq(log(lambda.pcmc.max), log(5*10^-2*lambda.pcmc.max), length = L))
...
roc.pcmc.bglasso <- foreach(l = 1:L, .combine="cbind", .packages="matrixcalc") %dopar% {
  G.bglasso <- bglasso(S = S.pcmcopula+10^-3*rho.pcmc*diag(M*p), lambda = lambda.pcmc[l], n = n, p = p,
                       tol.Theta=tol.Theta, tol.beta=tol.beta,
                       tol.beta_k=tol.beta_k, maxit.Theta=maxit.Theta, maxit.beta=maxit.beta,
                       maxit.beta_k=maxit.beta_k, Theta=NULL, Theta.inv= NULL, method=method)$alpha
  TFPR.l <- roc(est = G.bglasso, pop = G, thres = 0)
...
save(df, file=paste(save.path,"/ROC_a_d03_exp.seed",rand.seed,".Rdata",sep=""))
...
lambda.copula = exp(seq(log(lambda.copula.max/2), log(lambda.copula.max/5), length = L))
...
save(df, file=paste(save.path,"/ROC_a_d05_exp.seed",rand.seed,".Rdata",sep=""))
...
save(df, file=paste(save.path,"/ROC_a_d07_exp.seed",rand.seed,".Rdata",sep=""))
...
lambda.pcmc = exp(seq(log(lambda.pcmc.max), log(10^-2*lambda.pcmc.max), length = L))
...
save(df, file=paste(save.path,"/ROC_a_d03_power.seed",rand.seed,".Rdata",sep=""))
...
save(df, file=paste(save.path,"/ROC_a_d05_power.seed",rand.seed,".Rdata",sep=""))
