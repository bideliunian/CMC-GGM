...
######################
U.df = read.csv(file=paste(read.path,"/proj_subspace_model1_exp_d10.csv",sep=""), header = FALSE)
U.list = list()
for (i in 1:p) {
...
lambda.cmc = exp(seq(log(lambda.cmc.max), log(10^-1*lambda.cmc.max), length = L))
lambda.pcmc = exp(seq(log(lambda.pcmc.max), log(10^-1*lambda.pcmc.max), length = L))
lambda.copula = exp(seq(log(lambda.copula.max), log(10^-1*lambda.copula.max), length = L))
...
###############################
df = data.frame(trans.methods = rep(c("pcmc","cmc","copula", "pcmc","cmc", "copula"),each=L),
                methods = rep(c("thres","bglasso"),each=3*L),
                TP = cbind(roc.pcmc.thres, roc.cmc.thres, roc.copula.thres, roc.pcmc.bglasso, 
                           roc.cmc.bglasso, roc.copula.bglasso)[1,],
                FP = cbind(roc.pcmc.thres, roc.cmc.thres, roc.copula.thres, roc.pcmc.bglasso, 
                           roc.cmc.bglasso, roc.copula.bglasso)[2,])

save(df, file=paste(save.path,"/ROC_a_d10_exp.seed",rand.seed,".Rdata",sep=""))
