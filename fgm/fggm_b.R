...
# save.path <- "D:/VcopularGGM/Codes/fda/Results/ModelB"

## using aci
func.path <- "~/work/CMC-GGM/Functions"
working.path <- "~/work/CMC-GGM/fda"
save.path <- "~/work/CMC-GGM/fda/Results/ModelB"
...
for (M in M.list){
  ...
  save(roc.df, file=paste(save.path,"/ROC.B.seed",args,"M", M, ".Rdata",sep=""))  
}
