...
working.path <- "~/work/CMC-GGM/Model_a"
save.path <- "~/work/CMC-GGM/Model_a/Results/p50"
...
###########################
data = gen_data(n=n, p=p, m=M, model=model, run.ind = rand.seed, trans.type = trans.type)
data.X = data$X
G = data$Omega0
Omega = data$Omega
...
save(df, file=paste(save.path,"/ROC.a.050.gauss.seed",rand.seed,".Rdata",sep=""))
...
save(df, file=paste(save.path,"/ROC.a.050.copula.seed",rand.seed,".Rdata",sep=""))
...
est.prec.linear.thres = vggm.thr(X=S.linear, p = p, rho=10^-3, ridge = TRUE)$Theta0
...
save(df, file=paste(save.path,"/ROC.a.050.cmcopula.seed",rand.seed,".Rdata",sep=""))
